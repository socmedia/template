            ...
            @if (is_array($images))
                ...
                @if (count($images) == 0)
                    <div class="file-upload-input" style="height: {{ $height }}">
                        ...
                        <input type="file" name="images" id="images" wire:model="images" accept="image/*"
                            {{ !$multiple ?: 'multiple' }}>
                    </div>
                @endif
                ...
                @if ($multiple)
                    ...
                    @foreach ($images->temporaryUrl() as $i => $image)
                        <div class="file-upload-img" style="height: {{ $height }}">
                            <img src="{{ $image }}" alt="" />
                            <button type="button" class="remove" wire:click="removeImage(true, {{ $i }})"><i
                ...
                @else
                    @if ($images->temporaryUrl())
                        <div class="file-upload-img" style="height: {{ $height }}">
                            ...
                            <button type="button" class="remove" wire:click="removeImage"><i
                                    class="bx bx-x"></i></button>
                        </div>
